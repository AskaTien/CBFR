half3 Translucency(float3 viewDirectionWS, Light light, float amount) {
...
half3 ApplyLighting(SurfaceData surfaceData, Light mainLight, half3 vertexLight, float2 lightmapUV, half3 normalWS, half3 positionWS, half translucency)
{
	...
	for (uint i = 0u; i < additionalLightsCount; ++i)
	{
		...
		// Apply tranlucency for additional lights?
		//color += Translucency(viewDirectionWS, light, translucency);
#else
	...
	float3 transmissionColor = Translucency(viewDirectionWS, mainLight, translucency);

	color += BlendOverlay(transmissionColor * 2.0, color);
