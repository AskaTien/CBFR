...
float4 ApplyVertexColor(in float4 vertexPos, in float3 wPos, in float3 baseColor, in float mask, in float aoAmount, in float darkening, in float4 hue, in float posOffset)
{
	...
	float4 colormap = lerp(col, SampleColorMapTextureLOD(wPos), _ColorMapStrength);

	if (_ColorMapUV.w == 1) {
		float colormask = smoothstep(_ColorMapHeight, 1.0 + _ColorMapHeight, sqrt(mask));
		col.rgb = lerp(colormap.rgb, col.rgb, colormask);
	}
#endif
